...
describe('OrdersReducerTest', () => {
  ...
  it('onConnecting', () => {
    const state = ordersSlice.reducer(errorState, {type: onConnecting.type});
    expect(state).toEqual(initialState);
  });

  it('onOpen', () => {
    const state = ordersSlice.reducer(errorState, {type: onOpen.type});
    expect(state).toEqual(initialState);
  });

  it('onClose', () => {
    const state = ordersSlice.reducer(errorState, {type: onClose.type});
    expect(state).toEqual(initialState);
  });

  it('onError', () => {
    const state = ordersSlice.reducer(initialState, {type: onError.type, payload: 'Error'});
    expect(state).toEqual(errorState);
  });

  it('onMessage', () => {
    const state = ordersSlice.reducer(initialState, {type: onMessage.type, payload: message});
